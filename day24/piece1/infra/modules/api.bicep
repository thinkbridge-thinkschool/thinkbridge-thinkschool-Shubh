...
//
// Reused as-is from day23/piece1/infra/modules/api.bicep: Day 24's focus is the azd +
// Deployment Stacks delivery mechanism, not a new architecture, so the workload shape is
// unchanged. The real Quotes API (day22/piece1) already runs as a Container App inside a
// shared Container Apps Environment (Azure for Students allows exactly one managed environment
// per subscription, and it already exists: cae-yayuogblvizdw in rg-quotes-api). This module can
