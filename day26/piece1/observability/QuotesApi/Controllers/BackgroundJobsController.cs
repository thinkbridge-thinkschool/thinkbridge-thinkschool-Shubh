...
public sealed class BackgroundJobsController : ControllerBase
{
    ...
    //
    // The job body below does not run on this request's thread — QuoteBackgroundWorker
    // dequeues and invokes it later, from its own hosted-service loop, through an in-memory
    // Channel<T>. That channel is not an OpenTelemetry-instrumented transport, so
    // Activity.Current is null by the time the delegate runs there; without deliberately
    // carrying the trace context across that boundary, the worker's work would show up in
    // Application Insights as a disconnected trace with no link back to this request.
    //
        ...
        CancellationToken cancellationToken)
    {
        ...
        await _queue.QueueAsync(async stoppingToken =>
        {
            ...
            var quoteCount = await db.Quotes.CountAsync(stoppingToken);
