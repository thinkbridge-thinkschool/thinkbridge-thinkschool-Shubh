...
//
// Topology mirrors what Day 19 (ServiceBusDemo) and Day 20/22's transactional outbox relay
// actually use against the real `sb-day19-quotedemo` namespace: a single topic `quote-events`
// with two subscriptions, `sub-a` (competing consumers) and `sub-b` (independent consumer),
// each with MaxDeliveryCount=3 / LockDuration=PT30S. Topics require the Standard tier — Basic
// does not support topics/subscriptions at all.
//
// No connection string or SAS key is ever created or output here: the API's outbox relay
// authenticates purely via DefaultAzureCredential (see QuotesApi/Program.cs), so the only
// thing this module wires up beyond the messaging topology itself is an RBAC role assignment
// granting the API's managed identity data-plane access.
