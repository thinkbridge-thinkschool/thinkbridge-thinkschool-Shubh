        ...
        IHostEnvironment environment)
    {
        ...
        else
        {
            ...
            services.AddDbContext<QuotesDbContext>((sp, options) =>
            {
                ...
                // Day 30: local-only development against a local SQL Server (see
                // SqlOptions.LocalConnectionString) — plain SQL auth, no Managed Identity
                // interceptor, so this never depends on (or reaches) Azure. Supplied only via
                // user-secrets/environment; appsettings.json never carries this key.
                if (!string.IsNullOrWhiteSpace(sqlOptions.LocalConnectionString))
                {
                    options.UseSqlServer(sqlOptions.LocalConnectionString,
