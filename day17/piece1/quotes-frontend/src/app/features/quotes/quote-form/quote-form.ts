  ...
  protected async handleSubmit(event: Event): Promise<void> {
    ...
    const ok = await submit(this.quoteForm, async (_field, { submitted }) => {
      ...
      try {
        ...
        // The httpErrorMappingInterceptor (app.config.ts) has already turned
        // any HttpErrorResponse into an AppError by the time it reaches here;
        // the isAppError fallback only guards against a non-HTTP rejection.
        const appError: AppError = isAppError(err)
          ? err
          : {
              kind: 'server',
              message: 'Something went wrong while creating the quote. Your entries were kept — please try again.',
              status: 0,
            };
        return this.mapServerError(appError, submitted);
      }
    });
