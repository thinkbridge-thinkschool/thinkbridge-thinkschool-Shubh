...
describe('QuotesList states', () => {
  ...
  it('does not let a stale page response overwrite the list once the user has paged further ahead', async () => {
    const fixture = TestBed.createComponent(QuotesList);
    fixture.detectChanges();

    // Full page of 10 so the "Next" button is enabled once this resolves.
    const page1 = Array.from({ length: 10 }, (_, i) => ({
      id: i + 1,
      author: `Author ${i + 1}`,
      text: `Page 1 quote ${i + 1}`,
      isDeleted: false,
      userId: i + 1,
    }));
    expectPageRequest(1).flush(page1);
    await fixture.whenStable();
    fixture.detectChanges();

    const el = fixture.nativeElement as HTMLElement;
    const nextButton = () => el.querySelector('.pagination button:last-of-type') as HTMLButtonElement;

    // Page -> 2: request goes out but is left pending.
    nextButton().click();
    await fixture.whenStable();
    fixture.detectChanges();
    const page2Req = expectPageRequest(2);

    // Page -> 3 before page 2's response arrives: a second request goes out.
    nextButton().click();
    await fixture.whenStable();
    fixture.detectChanges();
    const page3Req = expectPageRequest(3);

    // Newer request (page 3) resolves first.
    page3Req.flush([{ id: 30, author: 'Page 3 author', text: 'Page 3 quote', isDeleted: false, userId: 30 }]);
    await fixture.whenStable();
    fixture.detectChanges();
    expect(el.textContent).toContain('Page 3 quote');

    // Older request (page 2) resolves after — must be ignored, not clobber page 3's data.
    page2Req.flush([{ id: 20, author: 'Page 2 author', text: 'Page 2 quote', isDeleted: false, userId: 20 }]);
    await fixture.whenStable();
    fixture.detectChanges();

    expect(el.textContent).toContain('Page 3 quote');
    expect(el.textContent).not.toContain('Page 2 quote');
  });
});
