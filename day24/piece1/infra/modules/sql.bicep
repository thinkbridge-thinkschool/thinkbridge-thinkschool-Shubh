...
//
// Reused as-is from day23/piece1/infra/modules/sql.bicep. This describes the Azure SQL
// infrastructure the Quotes API would need to move to a real, durable, shared database —
// the API's actual current runtime (day22/piece1) uses local SQLite, not this — mirroring the
// general-purpose serverless pattern already used elsewhere in this subscription (an existing
// `day8-index-shubh-2026` SQL server, kept untouched; this deploys its own separate
// server/database per environment instead of adopting that one).
@minLength(1)
@maxLength(63)
...
@secure()
@description('SQL auth administrator password. Must never appear as a literal in a checked-in parameter file — supply via a process environment variable at deploy time.')
